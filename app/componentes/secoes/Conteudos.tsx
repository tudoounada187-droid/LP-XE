import { conteudos } from "@/dados/conteudos";
      ...
      <div className="container-x relative z-10">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {conteudos.map((conteudo) => (
            <article key={conteudo.titulo} className="brand-card rounded-card p-6 transition-transform duration-300 hover:-translate-y-1">
              <p className="font-mono text-xs font-semibold uppercase text-ink-soft">{conteudo.data}</p>
              <h3 className="mt-10 text-xl font-bold">{conteudo.titulo}</h3>
              <p className="mt-3 leading-7 text-ink-soft">{conteudo.resumo}</p>
            </article>
          ))}
        </div>
      </div>
